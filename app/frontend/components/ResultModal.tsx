...
interface ResultModalProps {
  ...
  type:         "success" | "error";
  avatarName?:  string;
  avatarPrice:  number;   // prix de l'avatar
  userCoins:    number;   // solde AVANT achat
  newBalance:   number;   // solde APRÈS achat (ou inchangé si erreur)
}

export default function ResultModal({
  visible, onClose, type, avatarName,
  avatarPrice, userCoins, newBalance,
}: ResultModalProps) {
  const isSuccess = type === "success";
  const missing   = avatarPrice - userCoins; // pièces manquantes si erreur
            ...
            <View style={styles.statusFrame}>
              ...
              <View style={styles.textContent}>
                <Text style={styles.mainTitle}>
                  {isSuccess ? "Achat confirmé !" : "Pièces insuffisantes !"}
                </Text>
                <Text style={styles.subText}>
                  {isSuccess
                    ? `Tu as bien acheté l'avatar "${avatarName}".\nTu peux le retrouver dans "Mes avatars"`
                    : `Il te manque ${missing} pièce${missing > 1 ? "s" : ""} pour acheter cet avatar !`}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.infoOutsideArea}>
              {isSuccess ? (
                <View style={styles.successInfoBox}>
                  <CoinPrice
                    price={avatarPrice.toLocaleString()}
                    isMinus
                    colors={["#7A5CC0", "#7A5CC0"]}
                    containerStyle={{ height: "100%", borderRadius: 25 }}
                  />
                  ...
                  <View style={styles.errorRow}>
                    ...
                    <View>
                      <Text style={styles.errorLabel}>
                        Ton solde :{" "}
                        <Text style={styles.errorValue}>{userCoins.toLocaleString()}</Text>
                      </Text>
                      <Text style={styles.errorLabel}>
                        Prix avatar :{" "}
                        <Text style={styles.errorValue}>{avatarPrice.toLocaleString()}</Text>
                      </Text>
                    </View>
                  </View>
                </View>
                  ...
                  <>
                    ...
                    <TouchableOpacity style={styles.tabFlex} onPress={() => setSelectedTab("recharge")}>
                      {selectedTab === "recharge" ? (
                        <View style={styles.whiteStrokeWrapper}>
                          <LinearGradient colors={["#765EFF", "#D4BAF9"]} style={styles.activeTabBtnGradient}>
                            <Text style={styles.activeTabText}>Recharger mes pièces</Text>
                          </LinearGradient>
                        </View>
                      ) : (
                        <Text style={styles.inactiveTabText}>Recharger mes pièces</Text>
                      )}
                    </TouchableOpacity>
                  </>
                )}
