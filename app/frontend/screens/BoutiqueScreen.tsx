...
const AvatarRenderer = React.memo(({ model, grayscale = false }: { model: any, grayscale?: boolean }) => {
  ...
  const html = useMemo(() => {
    ...
    return `
      ...
      <head>
        <style>
          body { margin: 0; background: #ffffff; overflow: hidden; filter: ${grayscale ? 'grayscale(1) brightness(0.7)' : 'none'}; }
    ...
    return (
      <View style={[styles.card, isLocked && styles.cardLocked]}>
        {!isBoutique && (
          <>
            {item.active ? (
              <View style={[styles.statusBadge, styles.badgeActive]}><Text style={styles.badgeText}>Actif</Text></View>
            ) : item.owned ? (
              <View style={[styles.statusBadge, styles.badgeOwned]}><Text style={styles.badgeText}>Possédé</Text></View>
            ) : null}
            {isLocked && (
              <View style={styles.lockBadge}><Ionicons name="lock-closed" size={12} color="white" /></View>
            )}
          </>
        )}

        <View style={styles.avatarContainer}>
          <AvatarRenderer model={item.model} grayscale={isLocked} />
          {item.active && !isBoutique && (
            <View style={styles.checkIcon}><Ionicons name="checkmark-circle" size={20} color="#765EFF" /></View>
          )}
        </View>
    ...
    <LinearGradient colors={["#F0F4FF", "#FFFFFF"]} style={styles.container}>
      ...
      <View style={styles.profileArea}>
        <View style={styles.profileCircle}>
           <AvatarRenderer model={activeAvatar?.model} />
        </View>
      </View>
      ...
      <View style={styles.tabsWrapper}>
        <View style={styles.tabsPill}>
          <TouchableOpacity style={styles.tabItem} onPress={() => setActiveTab('mes_avatars')}>
            <LinearGradient colors={activeTab === 'mes_avatars' ? ["#BAAAE7", "#6949A8"] : ["transparent", "transparent"]} style={styles.tabGradient}>
          ...
          <TouchableOpacity style={styles.tabItem} onPress={() => setActiveTab('boutique')}>
            <LinearGradient colors={activeTab === 'boutique' ? ["#BAAAE7", "#6949A8"] : ["transparent", "transparent"]} style={styles.tabGradient}>
      ...
      />
      <ResultModal visible={resultVisible} type={modalType} avatarName={selectedAvatar?.name} onClose={() => setResultVisible(false)} />
