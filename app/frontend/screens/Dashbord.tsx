    ...
    <View style={headerStyles.container}>
      <View style={headerStyles.topRow}>
        ...
        <View style={headerStyles.coinsBadge}>
          <GoldCoin size={20} />
          <Text style={headerStyles.coinsText}>
            {(globalStats.gold > 0 ? globalStats.gold : USER.coins).toLocaleString()}
          </Text>
        </View>
...
const StatsCard = () => {
  ...
  const STATS: Stat[] = [
    { label: "Énergie",      percent: localStats.energie,      color: COLORS.statEnergie,      emoji: "⚡" },
    { label: "Stress",       percent: localStats.stress,       color: COLORS.statStress,       emoji: "😰" },
    { label: "Connaissance", percent: localStats.connaissance, color: COLORS.statConnaissance, emoji: "📚" },
    { label: "Organisation", percent: localStats.organisation, color: COLORS.statOrganisation, emoji: "🗂️" },
  ];
      ...
      <View style={statsStyles.energyBtns}>
        ...
        >
          <View style={statsStyles.sleepBtnRow}>
            ...
            <View>
              <Text style={statsStyles.sleepBtnText}>{sleeping ? "En cours..." : "Dormir"}</Text>
        ...
        <TouchableOpacity
          style={[
            statsStyles.potionBtn,
            potionQty > 0 && { backgroundColor: "#e8f5e9", borderColor: COLORS.statOrganisation },
          ]}
        ...
        >
          <View style={statsStyles.sleepBtnRow}>
            ...
            <View>
              ...
              {/* ── Prix en GoldCoin ── */}
              {potionQty > 0 ? (
                <Text style={statsStyles.sleepBtnSub}>Appuyer pour utiliser</Text>
              ) : (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 3, marginTop: 2 }}>
                  <Text style={statsStyles.sleepBtnSub}>{ENERGY_POTION.price}</Text>
                  <GoldCoin size={12} />
                  <Text style={statsStyles.sleepBtnSub}>gold</Text>
                </View>
              )}
            </View>
          </View>
      ...
      <Modal visible={potionModal} transparent animationType="fade">
        <Pressable style={statsStyles.overlay} onPress={() => setPotionModal(false)}>
          <View style={statsStyles.modalBox}>
            ...
            {/* Prix avec GoldCoin */}
            <View style={statsStyles.modalPriceRow}>
              <Text style={statsStyles.modalPriceLabel}>Prix :</Text>
              <Text style={statsStyles.modalPriceValue}>{ENERGY_POTION.price}</Text>
              <GoldCoin size={18} />
              <Text style={statsStyles.modalPriceValue}>gold</Text>
            </View>
      ...
      <Modal visible={sleepModal} transparent animationType="slide">
        <View style={statsStyles.overlay}>
          <View style={[statsStyles.modalBox, { paddingBottom: 28 }]}>
            ...
            <TouchableOpacity onPress={() => setSleepModal(false)} style={{ marginTop: 16 }}>
              <Text style={statsStyles.modalCancel}>Annuler</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const statsStyles = StyleSheet.create({
  ...
  modalPriceRow:     { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 16 },
  modalPriceLabel:   { fontSize: 15, color: "#888" },
  modalPriceValue:   { fontSize: 15, fontWeight: "800", color: COLORS.secondary },
  ...
  return (
    <View style={[
      ...
      {isDone ? (
        ...
        >
          <Text style={missionStyles.btnText}>{isContinue ? "Continuer ▶" : "Démarrer ▶"}</Text>
        </TouchableOpacity>
      )}
    </View>
  );
  ...
  return (
    <View style={[missionsStyles.card, SHADOWS.light]}>
      ...
      <TouchableOpacity
        ...
        onPress={() => router.push("/frontend/screens/Missions?openCreate=true")}
      >
...
const BossEventBanner = () => {
  ...
  const [bossData, setBossData] = useState({
    activeCount: 0, progression: 0, totalXp: 0, loading: true,
  });
    ...
    const load = async () => {
      try {
        const { data: bossEvents, error } = await supabase
          ...
          .select(`
            id_boss, nom,
            mission ( id_mission, id_user, xp_gain, mission_validation ( statut ) )
          `)
          .order("created_at", { ascending: false });

        if (error) throw error;

        let totalMissions = 0, doneMissions = 0, activeCount = 0, totalXp = 0;
        ...
        (bossEvents ?? []).forEach((boss: any) => {
          ...
          userMissions.forEach((m: any) => {
            totalMissions++;
            const isDone = (m.mission_validation ?? []).some((v: any) => v.statut === "done");
            if (isDone) { doneMissions++; totalXp += m.xp_gain ?? 0; }
          });
        });

        const progression = totalMissions > 0 ? Math.round((doneMissions / totalMissions) * 100) : 0;
  ...
  if (bossData.loading || bossData.activeCount === 0) return null;
    ...
    <View style={bossStyles.outer}>
      <View style={bossStyles.topBanner}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={bossStyles.bossSub}>{bossData.activeCount} actif{bossData.activeCount > 1 ? "s" : ""} 🔥</Text>
        </View>
        <View style={bossStyles.xpChip}>
          <Text style={bossStyles.xpText}>+{bossData.totalXp} XP</Text>
        </View>
        <TouchableOpacity style={bossStyles.voirBtn} onPress={() => router.push("/frontend/screens/EventsScreen")}>
      ...
      <View style={bossStyles.bottomCard}>
        ...
        <View style={{ flex: 1, marginLeft: 14 }}>
          ...
          <View style={bossStyles.bottomRow2}>
            <Text style={bossStyles.bottomSub}>{bossData.activeCount} actif{bossData.activeCount > 1 ? "s" : ""}</Text>
...
export default function DashboardScreen() {
  ...
  const handleSuggestedMissionStart = (_mission: MissionSuggestion) => {};
