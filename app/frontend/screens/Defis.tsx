...
const PRIO_LABELS: Record<1 | 2 | 3 | 4, string> = {
  1: "Basse",
  2: "Normale",
  3: "Haute",
  4: "Urgente",
...
const IconSport = () => (
  <Svg width={28} height={28} viewBox="0 0 24 24" fill="none">
    ...
    <Path
      d="M12 3C8 7 8 17 12 21M12 3C16 7 16 17 12 21M3 12h18"
      stroke="#fff"
      strokeWidth={1.8}
      strokeLinecap="round"
    />
        ...
        <View style={styles.cardInner}>
          ...
          <View style={styles.cardContent}>
            ...
            <View style={styles.avatarRow}>
              {Array.from({ length: Math.min(defi.participants, 3) }).map(
                (_, i) => (
                  <AvatarCircle
                    key={i}
        ...
        <View style={styles.cardProgressTrack}>
          <View
            style={[
              ...
              { width: `${defi.progression}%` as any },
            ]}
          />
        </View>
        <Text style={styles.cardProgressLabel}>
          {defi.progression}% accompli
        </Text>
      </TouchableOpacity>
          ...
          <View style={modal.dateModalOverlay}>
            <View style={modal.dateModalCard}>
              ...
              <View style={modal.dateModalActions}>
                <TouchableOpacity
                  style={modal.dateModalCancel}
                  onPress={() => setShow(false)}
                >
                  <Text style={modal.dateModalCancelText}>Annuler</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={modal.dateModalConfirm}
                  onPress={() => {
                    onConfirm(temp);
                    setShow(false);
                  }}
        ...
        <View style={modal.dropMenu}>
          {options.map((opt) => (
            <TouchableOpacity
              ...
              style={[
                modal.dropOption,
                value === opt.value && modal.dropOptionSelected,
              ]}
              onPress={() => {
                onChange(opt.value);
                setOpen(false);
            ...
            >
              <Text
                style={[
                  modal.dropOptionText,
                  value === opt.value && modal.dropOptionTextSelected,
                ]}
              >
                {opt.label}
              </Text>
...
const EditModal = ({
  ...
  const loadMissions = async (id_defi: number) => {
    ...
    const { data, error } = await supabase
      .from("mission")
      .select(
        "id_mission, titre, description, duree_min, difficulte, priorite, xp_gain, date_limite",
      )
      // ✅ statut retiré
      .eq("id_defi", id_defi)
      .order("id_mission");
      ...
      for (const m of missions) {
        ...
        if (m.id_mission > 0)
          await supabase
            ...
            .update(payload)
            .eq("id_mission", m.id_mission);
        else await supabase.from("mission").insert(payload);
      }
        ...
        <View style={modal.container}>
          ...
          >
            ...
            <ModalField
              ...
              value={nom}
              onChange={setNom}
              placeholder="Ex : 30 jours de lecture"
            />
            <ModalField
              label="Description"
              value={description}
              onChange={setDescription}
              placeholder="Decris l'objectif..."
              multiline
            />
                ...
                return (
                  ...
                  >
                    <View
                      style={[
                        modal.iconCircle,
                        active && modal.iconCircleActive,
                      ]}
                    >
                      <IC />
                    </View>
                    <Text
                      style={[modal.iconLabel, active && modal.iconLabelActive]}
                    >
                      {ICON_LABELS[k]}
                    </Text>
                  </TouchableOpacity>
            ...
            <View style={modal.row2}>
              {(["actif", "en_attente", "termine"] as const).map((s) => (
                <TouchableOpacity
                  ...
                  style={[
                    modal.statutBtn,
                    statut === s && modal.statutBtnActive,
                  ]}
                  onPress={() => setStatut(s)}
                >
                  <Text
                    style={[
                      modal.statutBtnText,
                      statut === s && modal.statutTextActive,
                    ]}
                  >
                    {s === "actif"
                      ? "En cours"
            ...
            {loadingM ? (
              ...
              <Text style={modal.emptyMissions}>
                Aucune mission. Ajoutes-en une !
              </Text>
            ) : (
              missions.map((m, i) => (
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={modal.missionMeta}>
                        {m.duree_min} min · {DIFF_LABELS[m.difficulte]} ·{" "}
                        {PRIO_LABELS[m.priorite]} · {m.xp_gain} XP
                      </Text>
                    </View>
                    <Text style={modal.chevron}>
                      {openIdx === i ? "▲" : "▼"}
                    </Text>
                  </TouchableOpacity>
                  ...
                  {openIdx === i && (
                    <View style={modal.missionBody}>
                      <ModalField
                        label="Titre *"
                        value={m.titre}
                        onChange={(v) => updateMission(i, { titre: v })}
                        placeholder="Titre de la mission"
                      />
                      <ModalField
                        label="Description"
                        value={m.description}
                        onChange={(v) => updateMission(i, { description: v })}
                        multiline
                      ...
                      <TextInput
                        ...
                        onChangeText={(v) =>
                          updateMission(i, { duree_min: parseInt(v) || 0 })
                        }
                      ...
                      {/* Date + Heure limite */}
                      <Text style={[modal.label, { marginTop: 8 }]}>
                      ...
                      <TouchableOpacity
                        style={modal.deleteMissionBtn}
                        onPress={() => deleteMission(i)}
                      >
                        <Text style={modal.deleteMissionText}>
...
export default function DefiScreen() {
  ...
  const calculerProgression = async (id_defi: number): Promise<number> => {
    ...
    const missionsDone = new Set(
      (validations ?? []).map((v: any) => v.id_mission),
    );
  ...
  const loadDefis = useCallback(async () => {
    ...
    const { data, error } = await getDefisByStatut(
      userId ?? 1,
      statutMap[activeTab],
    );
    if (!error && data) {
  ...
  const handleSaved = (updatedDefi: Defi) => {
    setDefis((prev) =>
      prev.map((d) => (d.id === updatedDefi.id ? updatedDefi : d)),
    );
  };
          ...
          <View style={styles.sectionHeaderRow}>
            <View>
              ...
              <Text style={styles.sectionSubtitle}>
                {filtered.length} defi{filtered.length !== 1 ? "s" : ""} en
                cours
              </Text>
            </View>
