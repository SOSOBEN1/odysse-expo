// ─────────────────────────────────────────────
// backend/viewmodels/StatsSkillsViewModel.ts
//
// Logique métier pour :
...
import { PlayerStatsModel, UserSkillModel } from '../models/UserModel';
import type { PlayerStats } from '../models/UserModel';

// ── Types ─────────────────────────────────────

type VMResult<T = unknown> =
...
const StatsSkillsViewModel = {
  ...
  async getOnboardingQuestions(): Promise<VMResult<OnboardingQuestion[]>> {
    try {
  ...
  async checkOnboardingStatus(userId: string): Promise<{ alreadyDone: boolean }> {
  ...
  ): Promise<VMResult<{ stats: PlayerStats; skills: SkillEntry[] }>> {
  ...
  async getPlayerStats(userId: string): Promise<VMResult<PlayerStats>> {
    try {
  ...
  async getUserSkills(userId: string): Promise<VMResult> {
    try {
  ...
  _computeStats(questions: OnboardingQuestion[], answers: ResponseEntry[]): PlayerStats {
    const stats: PlayerStats = { ...PlayerStatsModel.defaultStats };

    // Créer un index rapide : question_id → option_id choisie
  ...
  _computeInitialSkills(
    _questions: OnboardingQuestion[],
    _answers:   ResponseEntry[],
    stats:      PlayerStats,
  ): SkillEntry[] {
...
export default StatsSkillsViewModel;